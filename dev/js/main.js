$(document).ready(function() {
	...
	$('a[href^="#"]').on('click', function (e) {
		...
		if (target.slice(1) == "intro") {
			...
			$scrollDist = $(window).height() - $("nav").outerHeight(true)
		}
	...
	$("#dev > div").hover(
        function() { $(this).find("div > p").css("transform", "translateX(0)"); },
        function() { $(this).find("div > p").css("transform", "translateX(-100%)"); }
    );

	$("#dev > div").click(
		function() {
			$("#dev > div:nth-child(2n)").css("max-height", "0");
        	$(this).next().css("max-height", "5em");
        }
    );
